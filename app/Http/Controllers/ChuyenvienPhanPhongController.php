...
class ChuyenvienPhanPhongController extends Controller
{
    public function index(Request $request)
    {
        ...
        $phanphongs = Phanphong::with('dothocs', 'khoahocs')->search($sophong, $dothoc, $mssv, $ten)->get();
        $soThuTu1 = 1;
        foreach ($phanphongs as $phanphong) {
            $phanphong->soThuTu1 = $soThuTu1;
            $soThuTu1++;
        }
        return view('chuyenvien.phanphong.phanphong')->with(compact('sophong', 'dothoc', 'mssv', 'ten', 'phongs', 'dothocs', 'khoahocs', 'khoas', 'lops', 'phandot', 'phanphongs'));
    }
    ...
    public function cvxoaPhanphong(Request $request)
    {
        if ($request->has('cvxoa_tat_ca_phan_phong')) {
            ...
            $ids = $request->input('cvphanphong_ids');
